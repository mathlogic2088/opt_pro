...
namespace MVSDK
{
    ...
    //SDK内部显示接口的显示方式
    internal enum emSdkDisplayMode
    {
    ...
    //图像的镜像操作
    internal enum emSdkMirrorDirection
    {
    ...
    //自动曝光时抗频闪的频闪
    internal enum emSdkLightFrequency
    {
    ...
    internal enum emSdkParameterTeam
    {
    ...
    internal enum emSdkParameterMode
    {
    ...
    //SDK生成的相机配置页面掩码值
    internal enum emSdkPropSheetMask
    {
    ...
    //SDK生成的相机配置页面的回调消息类型
    internal enum emSdkPropSheetMsg
    {
    ...
    internal struct RGBQUAD
    {
     ...
     [Serializable]
    //相机的设备信息，只读信息，请勿修改
    internal struct tSdkCameraDevInfo
    {
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acProductSeries; // 产品系列
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acProductName;    // 产品名称
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acFriendlyName;   // 昵称，后加#和索引号来区分
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acLinkName;       // 设备符号连接名，内部使用
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDriverVersion;  // 驱动版本
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acSensorType;     // sensor类型
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acPortType;       // 接口类型  
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acSn;             // 产品唯一序列号
        internal uint uInstance;        // 该型号相机在该电脑上的实例索引号，用于区分同型号多相机

    }

    //相机的分辨率设定范围
    internal struct tSdkResolutionRange
    {
        internal int iHeightMax;            //图像最大高度
        internal int iHeightMin;            //图像最小高度
        internal int iWidthMax;             //图像最大宽度
        internal int iWidthMin;             //图像最小宽度
        internal int uSkipModeMask;         //SKIP模式掩码，为0，表示不支持SKIP 。bit0为1,表示支持SKIP 2x2 ;bit1为1，表示支持SKIP 3x3....
        internal int uBinSumModeMask;       //BIN(求和)模式掩码，为0，表示不支持BIN 。bit0为1,表示支持BIN 2x2 ;bit1为1，表示支持BIN 3x3....
        internal int uBinAverageModeMask;   //BIN(求均值)模式掩码，为0，表示不支持BIN 。bit0为1,表示支持BIN 2x2 ;bit1为1，表示支持BIN 3x3....
        internal int uResampleMask;         //硬件重采样的掩码
    }

    //相机的分辨率描述
    internal struct tSdkImageResolution
    {
        internal int iIndex;               // 索引号，[0,N]表示预设的分辨率(N 为预设分辨率的最大个数，一般不超过20),OXFF 表示自定义分辨率(ROI)
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;        // 该分辨率的描述信息。仅预设分辨率时该信息有效。自定义分辨率可忽略该信息
        internal uint uBinSumMode;          // BIN(求和)的模式,范围不能超过tSdkResolutionRange中uBinSumModeMask
        internal uint uBinAverageMode;      // BIN(求均值)的模式,范围不能超过tSdkResolutionRange中uBinAverageModeMask
        internal uint uSkipMode;            // 是否SKIP的尺寸，为0表示禁止SKIP模式，范围不能超过tSdkResolutionRange中uSkipModeMask
        internal uint uResampleMask;        // 硬件重采样的掩码
        internal int iHOffsetFOV;        // 采集视场相对于Sensor最大视场左上角的垂直偏移
        internal int iVOffsetFOV;        // 采集视场相对于Sensor最大视场左上角的水平偏移
        internal int iWidthFOV;          // 采集视场的宽度 
        internal int iHeightFOV;         // 采集视场的高度
        internal int iWidth;             // 相机最终输出的图像的宽度
        internal int iHeight;            // 相机最终输出的图像的高度
        internal int iWidthZoomHd;       // 硬件缩放的宽度,不需要进行此操作的分辨率，此变量设置为0.
        internal int iHeightZoomHd;      // 硬件缩放的高度,不需要进行此操作的分辨率，此变量设置为0.
        internal int iWidthZoomSw;       // 软件缩放的宽度,不需要进行此操作的分辨率，此变量设置为0.
        internal int iHeightZoomSw;      // 软件缩放的高度,不需要进行此操作的分辨率，此变量设置为0.
    }

    //相机白平衡模式描述信息
    internal struct tSdkColorTemperatureDes
    {
        internal int iIndex;              // 模式索引号
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;    // 描述信息
    }

    //相机帧率描述信息
    internal struct tSdkFrameSpeed
    {
        internal int iIndex;            // 帧率索引号，一般0对应于低速模式，1对应于普通模式，2对应于高速模式      
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;  // 描述信息      
    }

    //相机曝光功能范围定义
    internal struct tSdkExpose
    {
        internal uint uiTargetMin;       //自动曝光亮度目标最小值     
        internal uint uiTargetMax;       //自动曝光亮度目标最大值         
        internal uint uiAnalogGainMin;   //模拟增益的最小值，单位为fAnalogGainStep中定义      
        internal uint uiAnalogGainMax;   //模拟增益的最大值，单位为fAnalogGainStep中定义        
        internal float fAnalogGainStep;   //模拟增益每增加1，对应的增加的放大倍数。例如，uiAnalogGainMin一般为16，fAnalogGainStep一般为0.125，那么最小放大倍数就是16*0.125 = 2倍
        internal uint uiExposeTimeMin;   //手动模式下，曝光时间的最小值，单位:行。根据CameraGetExposureLineTime可以获得一行对应的时间(微秒),从而得到整帧的曝光时间    
        internal uint uiExposeTimeMax;   //手动模式下，曝光时间的最大值，单位:行        
    }

    //触发模式描述
    internal struct tSdkTrigger
    {
        internal int iIndex;             //模式索引号      
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;      //该模式的描述信息    
    }

    //传输分包大小描述(主要是针对网络相机有效)
    internal struct tSdkPackLength
    {
        internal int iIndex;        //分包大小索引号      
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription; //对应的描述信息     
        internal uint iPackSize;
    }

    //预设的LUT表描述
    internal struct tSdkPresetLut
    {
        internal int iIndex;             //编号     
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;    //描述信息
    }

    //AE算法描述
    internal struct tSdkAeAlgorithm
    {
        internal int iIndex;                 //编号   
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;        //描述信息
    ...
    //RAW转RGB算法描述
    internal struct tSdkBayerDecodeAlgorithm
    {
        internal int iIndex;                 //编号  
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;        //描述信息
    ...
    //帧率统计信息
    internal struct tSdkFrameStatistic
    {
        internal int iTotal;        //当前采集的总帧数（包括错误帧）
        internal int iCapture;      //当前采集的有效帧的数量    
        internal int iLost;         //当前丢帧的数量    
    }

    //相机输出的图像数据格式
    internal struct tSdkMediaType
    {
        internal int iIndex;                 //格式种类编号
        [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
        internal byte[] acDescription;          //描述信息
        internal uint iMediaType;             //对应的图像格式编码，如CAMERA_MEDIA_TYPE_BAYGR8，在本文件中有定义。
    }

    //伽马的设定范围
    internal struct tGammaRange
    {
        internal int iMin;       //最小值
        internal int iMax;       //最大值
    }

    //对比度的设定范围
    internal struct tContrastRange
    {
        internal int iMin;    //最小值
        internal int iMax;    //最大值
    }
    ...
    //RGB三通道数字增益的设定范围
    internal struct tRgbGainRange
    {
        internal int iRGainMin;   //红色增益的最小值
        internal int iRGainMax;   //红色增益的最大值
        internal int iGGainMin;   //绿色增益的最小值
        internal int iGGainMax;   //绿色增益的最大值
        internal int iBGainMin;   //蓝色增益的最小值
        internal int iBGainMax;   //蓝色增益的最大值
    }

    //饱和度设定的范围
    internal struct tSaturationRange
    {
        internal int iMin;    //最小值
        internal int iMax;    //最大值
    }
    ...
    //锐化的设定范围
    internal struct tSharpnessRange
    {
        internal int iMin;    //最小值
        internal int iMax;    //最大值
    }
    ...
    //ISP模块的使能信息
    internal struct tSdkIspCapacity
    {
        internal uint bMonoSensor;        //表示该型号相机是否为黑白相机,如果是黑白相机，则颜色相关的功能都无法调节
        internal uint bWbOnce;            //表示该型号相机是否支持手动白平衡功能  
        internal uint bAutoWb;            //表示该型号相机是否支持自动白平衡功能
        internal uint bAutoExposure;      //表示该型号相机是否支持自动曝光功能
        internal uint bManualExposure;    //表示该型号相机是否支持手动曝光功能
        internal uint bAntiFlick;         //表示该型号相机是否支持抗频闪功能
        internal uint bDeviceIsp;         //表示该型号相机是否支持硬件ISP功能
        internal uint bForceUseDeviceIsp; //bDeviceIsp和bForceUseDeviceIsp同时为TRUE时，表示强制只用硬件ISP，不可取消。
        internal uint bZoomHD;            //相机硬件是否支持图像缩放输出(只能是缩小)。
    }

    /* 定义整合的设备描述信息，这些信息可以用于动态构建UI */
    internal struct tSdkCameraCapbility
    {
        internal IntPtr pTriggerDesc;
        internal int iTriggerDesc;           //触发模式的个数，即pTriggerDesc数组的大小

        internal IntPtr pImageSizeDesc;         //预设分辨率选择
        internal int iImageSizeDesc;         //预设分辨率的个数，即pImageSizeDesc数组的大小 

        internal IntPtr pClrTempDesc;           //预设色温模式，用于白平衡
        internal int iClrTempDesc;

        internal IntPtr pMediaTypeDesc;         //相机输出图像格式
        internal int iMediaTypdeDesc;        //相机输出图像格式的种类个数，即pMediaTypeDesc数组的大小。

        internal IntPtr pFrameSpeedDesc;        //可调节帧速类型，对应界面上普通 高速 和超级三种速度设置
        internal int iFrameSpeedDesc;        //可调节帧速类型的个数，即pFrameSpeedDesc数组的大小。

        internal IntPtr pPackLenDesc;           //传输包长度，一般用于网络设备
        internal int iPackLenDesc;           //可供选择的传输分包长度的个数，即pPackLenDesc数组的大小。 

        internal int iOutputIoCounts;        //可编程输出IO的个数
        internal int iInputIoCounts;         //可编程输入IO的个数

        internal IntPtr pPresetLutDesc;         //相机预设的LUT表
        internal int iPresetLut;             //相机预设的LUT表的个数，即pPresetLutDesc数组的大小

        internal int iUserDataMaxLen;        //指示该相机中用于保存用户数据区的最大长度。为0表示无。
        internal uint bParamInDevice;         //指示该设备是否支持从设备中读写参数组。1为支持，0不支持。

        internal IntPtr pAeAlmSwDesc;           //软件自动曝光算法描述
        internal int iAeAlmSwDesc;           //软件自动曝光算法个数

        internal IntPtr pAeAlmHdDesc;           //硬件自动曝光算法描述，为NULL表示不支持硬件自动曝光
        internal int iAeAlmHdDesc;           //硬件自动曝光算法个数，为0表示不支持硬件自动曝光

        internal IntPtr pBayerDecAlmSwDesc;     //软件Bayer转换为RGB数据的算法描述
        internal int iBayerDecAlmSwDesc;     //软件Bayer转换为RGB数据的算法个数

        internal IntPtr pBayerDecAlmHdDesc;     //硬件Bayer转换为RGB数据的算法描述，为NULL表示不支持
        internal int iBayerDecAlmHdDesc;     //硬件Bayer转换为RGB数据的算法个数，为0表示不支持

        /* 图像参数的调节范围定义,用于动态构建UI*/
        internal tSdkExpose sExposeDesc;      //曝光的范围值
        internal tSdkResolutionRange sResolutionRange; //分辨率范围描述  
        internal tRgbGainRange sRgbGainRange;    //图像数字增益范围描述  
        internal tSaturationRange sSaturationRange; //饱和度范围描述  
        internal tGammaRange sGammaRange;      //伽马范围描述  
        internal tContrastRange sContrastRange;   //对比度范围描述  
        internal tSharpnessRange sSharpnessRange;  //锐化范围描述  
        internal tSdkIspCapacity sIspCapacity;     //ISP能力描述

    }
     [Serializable]
    //图像帧头信息
    internal struct tSdkFrameHead
    {
        internal uint uiMediaType;       // 图像格式,Image Format
        internal uint uBytes;            // 图像数据字节数,Total bytes
        internal int iWidth;            // 宽度 Image height
        internal int iHeight;           // 高度 Image width
        internal int iWidthZoomSw;      // 软件缩放的宽度,不需要进行软件裁剪的图像，此变量设置为0.
        internal int iHeightZoomSw;     // 软件缩放的高度,不需要进行软件裁剪的图像，此变量设置为0.
        internal uint bIsTrigger;        // 指示是否为触发帧 is trigger 
        internal uint uiTimeStamp;       // 该帧的采集时间，单位0.1毫秒 
        internal uint uiExpTime;         // 当前图像的曝光值，单位为微秒us
        internal float fAnalogGain;       // 当前图像的模拟增益倍数
        internal int iGamma;            // 该帧图像的伽马设定值，仅当LUT模式为动态参数生成时有效，其余模式下为-1
        internal int iContrast;         // 该帧图像的对比度设定值，仅当LUT模式为动态参数生成时有效，其余模式下为-1
        internal int iSaturation;       // 该帧图像的饱和度设定值，对于黑白相机无意义，为0
        internal float fRgain;            // 该帧图像处理的红色数字增益倍数，对于黑白相机无意义，为1
        internal float fGgain;            // 该帧图像处理的绿色数字增益倍数，对于黑白相机无意义，为1
        internal float fBgain;            // 该帧图像处理的蓝色数字增益倍数，对于黑白相机无意义，为1
    }

    //图像帧描述
    internal struct tSdkFrame
    {
        internal tSdkFrameHead head;        //帧头
        internal IntPtr pBuffer;     //数据区
    }

    //图像捕获的回调函数定义
    internal delegate void CAMERA_SNAP_PROC(CameraHandle hCamera, IntPtr pFrameBuffer, ref tSdkFrameHead pFrameHead, IntPtr pContext);

    //SDK生成的相机配置页面的消息回调函数定义
    internal delegate void CAMERA_PAGE_MSG_PROC(CameraHandle hCamera, uint MSG, uint uParam, IntPtr pContext);
	...
	//			0：上次掉线原因，网络通讯失败
	//			1：上次掉线原因，相机掉电
	internal delegate void CAMERA_CONNECTION_STATUS_CALLBACK(CameraHandle hCamera, uint MSG,uint uParam,IntPtr pContext);


    // Grabber统计信息
    internal struct tSdkGrabberStat
    {
        internal int Width, Height;	// 帧图像大小
        internal int Disp;			// 显示帧数量
        internal int Capture;		    // 采集的有效帧的数量
        internal int Lost;			// 丢帧的数量
        internal int Error;			// 错帧的数量
        internal float DispFps;		// 显示帧率
        internal float CapFps;		// 捕获帧率
    }

    // Grabber图像捕获的回调函数定义
    internal delegate void pfnCameraGrabberFrameCallback(
        IntPtr Grabber,
        IntPtr pFrameBuffer,
    ...
    // Grabber帧监听函数定义
    internal delegate int pfnCameraGrabberFrameListener(
        IntPtr Grabber,
        int Phase,
        IntPtr pFrameBuffer,
        ref tSdkFrameHead pFrameHead,
    ...
    // Grabber异步抓图的回调函数定义
    internal delegate void pfnCameraGrabberSaveImageComplete(
        IntPtr Grabber,
    ...
    static internal class MvApi
    {
        internal static byte[] StringToCStr(string str)
        { 
            return Encoding.Default.GetBytes(str.Insert(str.Length, "\0"));
        }

        internal static string CStrToString(byte[] cstr)
        {
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSdkInit(
            int iLanguageSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraEnumerateDevice(
            IntPtr pCameraList,
            ref int piNums
        );

        internal static CameraSdkStatus CameraEnumerateDevice(IntPtr pCameraList, ref int piNums) 
        {
            return _CameraEnumerateDevice(pCameraList, ref piNums);
        }

        internal static CameraSdkStatus CameraEnumerateDevice(out tSdkCameraDevInfo[] CameraList)
        {
            int CameraCount = 256;
            CameraSdkStatus status;
            IntPtr ptr;
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraInit(
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetCallbackFunction(
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraUnInit(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetInformation(
            CameraHandle hCamera,
            ref IntPtr pbuffer
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImageProcess(
            CameraHandle hCamera,
            IntPtr pbyIn,
            IntPtr pbyOut,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraDisplayInit(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraDisplayRGB24(
            CameraHandle hCamera,
            IntPtr pbyRGB24,
        ref tSdkFrameHead pFrInfo
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetDisplayMode(
            CameraHandle hCamera,
            int iMode
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetDisplayOffset(
            CameraHandle hCamera,
            int iOffsetX,
            int iOffsetY
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetDisplaySize(
            CameraHandle hCamera,
            int iWidth,
            int iHeight
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetImageBuffer(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSnapToBuffer(
            CameraHandle hCamera,
        out tSdkFrameHead pFrameInfo,
        out IntPtr pbyBuffer,
            uint uWaitTimeMs
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraReleaseImageBuffer(
            CameraHandle hCamera,
            IntPtr pbyBuffer
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraPlay(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraPause(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraStop(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraInitRecord(
            CameraHandle hCamera,
            int iFormat,
            string pcSavePath,
            uint b2GLimit,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraStopRecord(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraPushFrame(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSaveImage(
            CameraHandle hCamera,
            string lpszFileName,
            IntPtr pbyImageBuffer,
        ref tSdkFrameHead pFrInfo,
        ...
        // 把帧图像转换成C# Image格式
        internal static System.Drawing.Image CSharpImageFromFrame(IntPtr pFrameBuffer, ref tSdkFrameHead tFrameHead)
        {
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetImageResolution(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetImageResolution(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetMediaType(
            CameraHandle hCamera,
        ref int piMediaType
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetMediaType(
            CameraHandle hCamera,
            int iMediaType
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeState(
            CameraHandle hCamera,
            uint bAeState
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeState(
            CameraHandle hCamera,
        ref uint pAeState
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetSharpness(
            CameraHandle hCamera,
            int iSharpness
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetSharpness(
            CameraHandle hCamera,
        ref int piSharpness
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetLutMode(
            CameraHandle hCamera,
            int emLutMode
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLutMode(
            CameraHandle hCamera,
        ref int pemLutMode
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSelectLutPreset(
            CameraHandle hCamera,
            int iSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLutPresetSel(
            CameraHandle hCamera,
        ref int piSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetCustomLut(
            CameraHandle hCamera,
            int iChannel,
            ushort[] pLut
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetCustomLut(
            CameraHandle hCamera,
            int iChannel,
        ref ushort[] pLut
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetCurrentLut(
            CameraHandle hCamera,
            int iChannel,
        ref ushort[] pLut
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetWbMode(
            CameraHandle hCamera,
            uint bAuto
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetWbMode(
            CameraHandle hCamera,
        ref uint pbAuto
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetPresetClrTemp(
            CameraHandle hCamera,
            int iSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetPresetClrTemp(
            CameraHandle hCamera,
        ref int piSel
        );
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetUserClrTempGain(
		  CameraHandle  hCamera,
		  int       iRgain,
		  int       iGgain,
		  int       iBgain
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraGetUserClrTempGain(
		  CameraHandle  hCamera,
		  out int      piRgain,
		  out int      piGgain,
		  out int      piBgain
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetUserClrTempMatrix(
		  CameraHandle  hCamera,
		  float[,]     pMatrix
		);
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraGetUserClrTempMatrix(
		  CameraHandle  hCamera,
		  float[,]      pMatrix
		);
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetClrTempMode(
		  CameraHandle  hCamera,
		  int       iMode
		);
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraGetClrTempMode(
		  CameraHandle  hCamera,
		  out int       pimode
		);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetOnceWB(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetOnceBB(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeTarget(
            CameraHandle hCamera,
            int iAeTarget
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeTarget(
            CameraHandle hCamera,
        ref int piAeTarget
        );
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeExposureRange(
			CameraHandle    hCamera, 
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeExposureRange(
			CameraHandle    hCamera, 
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeAnalogGainRange(
			CameraHandle    hCamera, 
			int				iMinAnalogGain,
			int				iMaxAnalogGain
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeAnalogGainRange(
			CameraHandle    hCamera, 
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeThreshold(
			CameraHandle    hCamera, 
			int				iThreshold
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeThreshold(
			CameraHandle    hCamera, 
			out int			iThreshold
			);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetExposureTime(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExposureLineTime(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExposureTime(
            CameraHandle hCamera,
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraGetExposureTimeRange(
			CameraHandle    hCamera, 
			ref double      pfMin,
			ref double      pfMax,
			ref double      pfStep
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAnalogGain(
            CameraHandle hCamera,
            int iAnalogGain
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAnalogGain(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetGain(
            CameraHandle hCamera,
            int iRGain,
            int iGGain,
            int iBGain
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetGain(
            CameraHandle hCamera,
        ref int piRGain,
        ref int piGGain,
        ref int piBGain
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetGamma(
            CameraHandle hCamera,
            int iGamma
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetGamma(
            CameraHandle hCamera,
        ref int piGamma
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetContrast(
            CameraHandle hCamera,
            int iContrast
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetContrast(
            CameraHandle hCamera,
        ref int piContrast
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetSaturation(
            CameraHandle hCamera,
            int iSaturation
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetSaturation(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetMonochrome(
            CameraHandle hCamera,
            uint bEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetMonochrome(
            CameraHandle hCamera,
        ref uint pbEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetInverse(
            CameraHandle hCamera,
            uint bEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetInverse(
            CameraHandle hCamera,
        ref uint pbEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAntiFlick(
            CameraHandle hCamera,
            uint bEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAntiFlick(
            CameraHandle hCamera,
        ref uint pbEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLightFrequency(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetLightFrequency(
            CameraHandle hCamera,
            int iFrequencySel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetFrameSpeed(
            CameraHandle hCamera,
            int iFrameSpeed
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetFrameSpeed(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetParameterMode(
            CameraHandle hCamera,
            int iTarget
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetParameterMode(
            CameraHandle hCamera,
        ref int piTarget
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetParameterMask(
            CameraHandle hCamera,
            uint uMask
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSaveParameter(
            CameraHandle hCamera,
            int iTeam
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSaveParameterToFile(
            CameraHandle hCamera,
            string sFileName
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraReadParameterFromFile(
            CameraHandle hCamera,
            string sFileName
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraLoadParameter(
            CameraHandle hCamera,
            int iTeam
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetCurrentParameterGroup(
            CameraHandle hCamera,
        ref int piTeam
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetTransPackLen(
            CameraHandle hCamera,
            int iPackSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetTransPackLen(
            CameraHandle hCamera,
        ref int piPackSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraIsAeWinVisible(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeWinVisible(
            CameraHandle hCamera,
            uint bIsVisible
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeWindow(
            CameraHandle hCamera,
        ref int piHOff,
        ref int piVOff,
        ref int piWidth,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeWindow(
            CameraHandle hCamera,
            int iHOff,
            int iVOff,
            int iWidth,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetMirror(
            CameraHandle hCamera,
            int iDir,
            uint bEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetMirror(
            CameraHandle hCamera,
            int iDir,
        ref uint pbEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetRotate(
	        CameraHandle    hCamera, 
	        int             iRot 
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetRotate(
	        CameraHandle    hCamera, 
	        out int         iRot 
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetWbWindow(
            CameraHandle hCamera,
        ref int PiHOff,
        ref int PiVOff,
        ref int PiWidth,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetWbWindow(
            CameraHandle hCamera,
            int iHOff,
            int iVOff,
            int iWidth,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraIsWbWinVisible(
            CameraHandle hCamera,
        ref uint pbShow
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetWbWinVisible(
            CameraHandle hCamera,
            uint bShow
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImageOverlay(
            CameraHandle hCamera,
            IntPtr pRgbBuffer,
        ref tSdkFrameHead pFrInfo
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetCrossLine(
            CameraHandle hCamera,
            int iLine,
            int x,
            int y,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetCrossLine(
            CameraHandle hCamera,
            int iLine,
        ref int px,
        ref int py,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetCapability(
            CameraHandle hCamera,
            IntPtr pCameraInfo
        );

        internal static CameraSdkStatus CameraGetCapability(CameraHandle hCamera, IntPtr pCameraInfo)
        {
            return _CameraGetCapability(hCamera, pCameraInfo);
        }

        internal static CameraSdkStatus CameraGetCapability(CameraHandle hCamera, out tSdkCameraCapbility cap)
        {
            CameraSdkStatus status;
            IntPtr ptr;
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraWriteSN(
            CameraHandle hCamera,
            Byte[] pbySN,
            int iLevel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraReadSN(
            CameraHandle hCamera,
            Byte[] pbySN,
            int iLevel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetTriggerDelayTime(
            CameraHandle hCamera,
            uint uDelayTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetTriggerDelayTime(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetTriggerCount(
            CameraHandle hCamera,
            int iCount
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetTriggerCount(
            CameraHandle hCamera,
        ref int piCount
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSoftTrigger(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetTriggerMode(
            CameraHandle hCamera,
            int iModeSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetTriggerMode(
            CameraHandle hCamera,
        ref int piModeSel
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetStrobeMode(
            CameraHandle hCamera,
            int iMode
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetStrobeMode(
            CameraHandle hCamera,
        ref int piMode
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetStrobeDelayTime(
            CameraHandle hCamera,
            uint uDelayTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetStrobeDelayTime(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetStrobePulseWidth(
            CameraHandle hCamera,
            uint uTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetStrobePulseWidth(
            CameraHandle hCamera,
        ref uint upTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetStrobePolarity(
            CameraHandle hCamera,
            int uPolarity
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetStrobePolarity(
            CameraHandle hCamera,
        ref int upPolarity
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetExtTrigSignalType(
            CameraHandle hCamera,
            int iType
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExtTrigSignalType(
            CameraHandle hCamera,
        ref int ipType
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetExtTrigShutterType(
            CameraHandle hCamera,
            int iType
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExtTrigShutterType(
            CameraHandle hCamera,
        ref int ipType
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetExtTrigDelayTime(
            CameraHandle hCamera,
            uint uDelayTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExtTrigDelayTime(
            CameraHandle hCamera,
        ref uint upDelayTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetExtTrigJitterTime(
            CameraHandle hCamera,
            uint uTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExtTrigJitterTime(
            CameraHandle hCamera,
        ref uint upTimeUs
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetExtTrigCapability(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetResolutionForSnap(
            CameraHandle hCamera,
        ref tSdkImageResolution pImageResolution
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetResolutionForSnap(
            CameraHandle hCamera,
        ref tSdkImageResolution pImageResolution
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraCustomizeResolution(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraCustomizeReferWin(
            CameraHandle hCamera,
            int iWintype,
            IntPtr hParent,
        ref int piHOff,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraShowSettingPage(
            CameraHandle hCamera,
            uint bShow
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraCreateSettingPage(
            CameraHandle hCamera,
            IntPtr hParent,
            byte[] pWintext,
        ...
        internal static CameraSdkStatus CameraCreateSettingPage(
            CameraHandle hCamera,
            IntPtr hParent,
            byte[] pWintext,
            CAMERA_PAGE_MSG_PROC pCallbackFunc,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetActiveSettingSubPage(
            CameraHandle hCamera,
            int index
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSpecialControl(
            CameraHandle hCamera,
            uint dwCtrlCode,
            uint dwParam,
            IntPtr lpData
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetFrameStatistic(
            CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetNoiseFilter(
            CameraHandle hCamera,
            uint bEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetNoiseFilterState(
            CameraHandle hCamera,
        ref uint pEnable
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraRstTimeStamp(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSaveUserData(
            CameraHandle hCamera,
            uint uStartAddr,
            Byte[] pbData,
            int ilen
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraLoadUserData(
            CameraHandle hCamera,
            uint uStartAddr,
            Byte[] pbData,
            int ilen
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetFriendlyName(
          CameraHandle hCamera,
          Byte[] pName
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetFriendlyName(
          CameraHandle hCamera,
          Byte[] pName
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSdkGetVersionString(
            Byte[] pVersionString
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraCheckFwUpdate(
          CameraHandle hCamera,
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetFirmwareVision(
          CameraHandle hCamera,
          Byte[] pVersion
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetEnumInfo(
          CameraHandle hCamera,
        ref tSdkCameraDevInfo pCameraInfo
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetInerfaceVersion(
          CameraHandle hCamera,
          Byte[] pVersion
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetIOState(
          CameraHandle hCamera,
          int iOutputIOIndex,
          uint uState
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetIOState(
          CameraHandle hCamera,
          int iInputIOIndex,
        ref uint puState
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetInPutIOMode(
			CameraHandle    hCamera,
			int         iInputIOIndex,
			int			iMode
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetOutPutIOMode(
			CameraHandle    hCamera,
			int         iOutputIOIndex,
			int			iMode
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetOutPutPWM(
			CameraHandle    hCamera,
			int         iOutputIOIndex,
			uint		iCycle,
			uint		uDuty
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAeAlgorithm(
          CameraHandle hCamera,
          int iIspProcessor,
          int iAeAlgorithmSel
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetAeAlgorithm(
          CameraHandle hCamera,
          int iIspProcessor,
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetBayerDecAlgorithm(
          CameraHandle hCamera,
          int iIspProcessor,
          int iAlgorithmSel
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetBayerDecAlgorithm(
          CameraHandle hCamera,
          int iIspProcessor,
        ref int piAlgorithmSel
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetIspProcessor(
          CameraHandle hCamera,
          int iIspProcessor
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetIspProcessor(
          CameraHandle hCamera,
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetBlackLevel(
          CameraHandle hCamera,
          int iBlackLevel
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetBlackLevel(
          CameraHandle hCamera,
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetWhiteLevel(
          CameraHandle hCamera,
          int iWhiteLevel
        );
        ...
        //        中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetWhiteLevel(
            CameraHandle hCamera,
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraIsOpened(
        ...
        // 返回值     : 返回设备个数，0表示无。
        /******************************************************/
        internal delegate int pfnCameraEnumerateDeviceEx(
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraInitEx(
            int iDeviceIndex,
            int iParamLoadMode,
            int emTeam,
        ref CameraHandle pCameraHandle
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraInitEx2(
            string CameraName,
        ...
        //            否则返回0。
        /******************************************************/
        internal delegate IntPtr pfnCameraGetImageBufferEx(
            CameraHandle hCamera,
        ref int piWidth,
        ref int piHeight,
            uint wTimes
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImageProcessEx(
            CameraHandle hCamera,
            IntPtr pbyIn,
            IntPtr pbyOut,
        ref tSdkFrameHead pFrInfo,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetIspOutFormat(
            CameraHandle hCamera,
            uint uFormat
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetIspOutFormat(
            CameraHandle hCamera,
        ref uint puFormat
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraReConnect(
            CameraHandle hCamera
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraConnectTest(
            CameraHandle hCamera
        );
		...
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetLedEnable(
			CameraHandle    hCamera,
			int             index,
			int             enable
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLedEnable(
			CameraHandle    hCamera,
			int             index,
			out int         enable
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetLedOnOff(
			CameraHandle    hCamera,
			int             index,
			int             onoff
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLedOnOff(
			CameraHandle    hCamera,
			int             index,
			out int         onoff
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetLedDuration(
			CameraHandle    hCamera,
			int             index,
			uint            duration
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLedDuration(
			CameraHandle    hCamera,
			int             index,
			out uint        duration
			);
        ...
        //            否则返回 非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetLedBrightness(
            CameraHandle hCamera,
            int index,
            uint uBrightness
        );
        ...
        //            否则返回 非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetLedBrightness(
			CameraHandle hCamera,
			int index,
        ...
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraEnableTransferRoi(
            CameraHandle hCamera,
            uint uEnableMask
        );
        ...
        //            其它非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetTransferRoi(
			CameraHandle hCamera,
			int index,
			uint X1,
			uint Y1,
        ...
        //            其它非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetTransferRoi(
            CameraHandle hCamera,
            int index,
            ref uint pX1,
            ref uint pY1,
        ...
        /******************************************************/
        internal delegate IntPtr pfnCameraAlignMalloc(
            int size,
            int align
        );
        ...
        // 返回值   : 无。
        /******************************************************/
        internal delegate void pfnCameraAlignFree(
            IntPtr membuffer
        );
        ...
        //            其它非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetAutoConnect(CameraHandle hCamera, int bEnable);
        ...
        //            其它非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetReConnectCounts(CameraHandle hCamera, ref uint puCounts);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraEvaluateImageDefinition(
            CameraHandle hCamera,
            int iAlgorithSel,
            IntPtr pbyIn,
            ref tSdkFrameHead pFrInfo,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraDrawText(
            IntPtr pRgbBuffer,
            ref tSdkFrameHead pFrInfo,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGigeGetIp(
            ref tSdkCameraDevInfo pCameraInfo,
            byte[] CamIp,
            byte[] CamMask,
            byte[] CamGateWay,
        ...
        internal static CameraSdkStatus CameraGigeGetIp(
            ref tSdkCameraDevInfo pCameraInfo,
            out string CamIp,
            out string CamMask,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGigeSetIp(
            ref tSdkCameraDevInfo pCameraInfo,
            string Ip,
            string SubMask,
            string GateWay,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGigeGetMac(
	        ref tSdkCameraDevInfo pCameraInfo,
	        byte[] CamMac,
	        byte[] EtMac
	    );

        internal static CameraSdkStatus CameraGigeGetMac(
            ref tSdkCameraDevInfo pCameraInfo,
            out string CamMac,
            out string EtMac
        )
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraEnableFastResponse(
	        CameraHandle hCamera
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetCorrectDeadPixel(
	        CameraHandle hCamera,
	        int bEnable
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetCorrectDeadPixel(
	        CameraHandle hCamera,
	        out int pbEnable
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraFlatFieldingCorrectSetEnable(
	        CameraHandle hCamera,
	        int bEnable
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraFlatFieldingCorrectGetEnable(
	        CameraHandle hCamera,
	        out int pbEnable
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraFlatFieldingCorrectSetParameter(
	        CameraHandle hCamera,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraFlatFieldingCorrectSaveParameterToFile(
	        CameraHandle hCamera,
	        string pszFileName
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraFlatFieldingCorrectLoadParameterFromFile(
	        CameraHandle hCamera,
	        string pszFileName
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraCommonCall(
	        CameraHandle    hCamera, 
	        byte[]		    pszCall,
	        byte[]	   		pszResult,
        ...
        internal static CameraSdkStatus CameraCommonCall(
            CameraHandle hCamera,
            string strCall,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetDenoise3DParams(
	        CameraHandle    hCamera, 
	        int			    bEnable,
	        int				nCount,
	        float[]			Weights
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetDenoise3DParams(
	        CameraHandle    hCamera, 
	        out int			bEnable,
	        out int         nCount,
	        out	int		    bUseWeight,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraManualDenoise3D(
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraCustomizeDeadPixels(
			CameraHandle	hCamera,
			IntPtr			hParent
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraReadDeadPixels(
			CameraHandle    hCamera,
			ushort[]		pRows,
			ushort[]		pCols,
			ref uint		pNumPixel
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraAddDeadPixels(
			CameraHandle    hCamera,
			ushort[]		pRows,
			ushort[]		pCols,
			uint			NumPixel
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraRemoveDeadPixels(
			CameraHandle    hCamera,
			ushort[]		pRows,
			ushort[]		pCols,
			uint			NumPixel
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraRemoveAllDeadPixels(
			CameraHandle    hCamera
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSaveDeadPixels(
			CameraHandle    hCamera
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSaveDeadPixelsToFile(
			CameraHandle    hCamera,
            string          sFileName
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraLoadDeadPixelsFromFile(
			CameraHandle    hCamera,
			string		    sFileName
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetImageBufferPriority(
			CameraHandle        hCamera, 
			out tSdkFrameHead   pFrameInfo, 
			out IntPtr          pbyBuffer,
			uint                wTimes,
		...
		//            否则返回0。
		/******************************************************/
        internal delegate IntPtr pfnCameraGetImageBufferPriorityEx(
			CameraHandle        hCamera, 
			out int             piWidth,
			out int             piHeight,
			uint                wTimes,
		...
		//            否则返回0。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetImageBufferPriorityEx2(
			CameraHandle    hCamera, 
			IntPtr          pImageData,
			uint            uOutFormat,
			out int         piWidth,
		...
		//            否则返回0。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetImageBufferPriorityEx3(
			CameraHandle hCamera, 
			IntPtr pImageData,
			uint uOutFormat,
			out int piWidth,
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraClearBuffer(
			CameraHandle hCamera
			);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSoftTriggerEx(
			CameraHandle hCamera,
			uint uFlags
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraSetHDR(
			CameraHandle    hCamera,
			float           value
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetHDR(
			CameraHandle    hCamera,
			out float       value
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetFrameID(
			CameraHandle    hCamera,
			out uint        id
			);
        ...
        //            否则返回 非0值，参考CameraStatus.h中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGetFrameTimeStamp(
	        CameraHandle    hCamera,
	        out uint        TimeStampL,
            out uint        TimeStampH
	        );
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetHDRGainMode(
			CameraHandle    hCamera,
			int				value
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraGetHDRGainMode(
			CameraHandle    hCamera,
			out int			value
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraDrawFrameBuffer(
			IntPtr pFrameBuffer, 
			ref tSdkFrameHead pFrameHead,
			IntPtr hWnd,
			int Algorithm,
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraFlipFrameBuffer(
			IntPtr pFrameBuffer, 
			ref tSdkFrameHead pFrameHead,
			int Flags
			);
		...
		//            否则返回 非0值，参考CameraStatus.h中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraConvertFrameBufferFormat(
			CameraHandle hCamera,
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetConnectionStatusCallback(
			CameraHandle        hCamera,
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetLightingControllerMode(
			CameraHandle        hCamera,
			int					index,
			int					mode
			);
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraSetLightingControllerState(
			CameraHandle        hCamera,
			int					index,
			int					state
			);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_CreateFromDevicePage(
            out IntPtr Grabber
        );
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_CreateByIndex(
			out IntPtr Grabber,
			int Index
		);
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_CreateByName(
			out IntPtr Grabber,
			string Name
		);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_Create(
            out IntPtr Grabber,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_Destroy(
            IntPtr Grabber
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SetHWnd(
            IntPtr Grabber,
            IntPtr hWnd
        );
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SetPriority(
			IntPtr Grabber,
			uint Priority
		);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_StartLive(
            IntPtr Grabber
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_StopLive(
            IntPtr Grabber
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SaveImage(
            IntPtr Grabber,
            out IntPtr Image,
            uint TimeOut
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SaveImageAsync(
            IntPtr Grabber
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SaveImageAsyncEx(
            IntPtr Grabber,
            IntPtr UserData
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SetSaveImageCompleteCallback(
            IntPtr Grabber,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SetFrameListener(
            IntPtr Grabber,
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SetRawCallback(
			IntPtr Grabber,
		...
		//            中错误码的定义。
		/******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_SetRGBCallback(
			IntPtr Grabber,
			pfnCameraGrabberFrameCallback Callback,
			IntPtr Context
			);
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_GetCameraHandle(
            IntPtr Grabber,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_GetStat(
            IntPtr Grabber,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraGrabber_GetCameraDevInfo(
            IntPtr Grabber,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_Create(
            out IntPtr Image,
            IntPtr pFrameBuffer,
            ref tSdkFrameHead pFrameHead,
            int bCopy
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_CreateEmpty(
	        out IntPtr Image
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_Destroy(
            IntPtr Image
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_GetData(
            IntPtr Image,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_GetUserData(
	        IntPtr Image,
	        out IntPtr UserData
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_SetUserData(
	        IntPtr Image,
	        IntPtr UserData
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_IsEmpty(
	        IntPtr Image,
	        out int IsEmpty
	    );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_Draw(
            IntPtr Image,
            IntPtr hWnd,
            int Algorithm
        );
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraImage_DrawFit(
			IntPtr Image,
			IntPtr hWnd,
			int Algorithm
			);
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraImage_DrawToDC(
			IntPtr Image,
			IntPtr hDC,
			int Algorithm,
			int xDst,
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraImage_DrawToDCFit(
			IntPtr Image,
			IntPtr hDC,
			int Algorithm,
			int xDst,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_BitBlt(
            IntPtr Image,
            IntPtr hWnd,
            int xDst,
            int yDst,
		...
		//            中错误码的定义。
		/******************************************************/
		internal delegate CameraSdkStatus pfnCameraImage_BitBltToDC(
			IntPtr Image,
			IntPtr hDC,
			int xDst,
			int yDst,
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_SaveAsBmp(
            IntPtr Image,
            string FileName
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_SaveAsJpeg(
            IntPtr Image,
            string FileName,
            byte Quality
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_SaveAsPng(
            IntPtr Image,
            string FileName
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_SaveAsRaw(
            IntPtr Image,
            string FileName,
            int Format
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate CameraSdkStatus pfnCameraImage_IPicture(
            IntPtr Image,
            out Object NewPic
        );
        ...
        //            中错误码的定义。
        /******************************************************/
        internal delegate string pfnCameraGetErrorString(
            CameraSdkStatus status
        );

        ////////////////////////////////////////////////////////////////////////////////////////////////

        internal static pfnCameraSdkInit CameraSdkInit;
        internal static pfnCameraEnumerateDevice _CameraEnumerateDevice;
        internal static pfnCameraInit CameraInit;
        internal static pfnCameraSetCallbackFunction CameraSetCallbackFunction;
        internal static pfnCameraUnInit CameraUnInit;
        internal static pfnCameraGetInformation CameraGetInformation;
        internal static pfnCameraImageProcess CameraImageProcess;
        internal static pfnCameraDisplayInit CameraDisplayInit;
        internal static pfnCameraDisplayRGB24 CameraDisplayRGB24;
        internal static pfnCameraSetDisplayMode CameraSetDisplayMode;
        internal static pfnCameraSetDisplayOffset CameraSetDisplayOffset;
        internal static pfnCameraSetDisplaySize CameraSetDisplaySize;
        internal static pfnCameraGetImageBuffer CameraGetImageBuffer;
        internal static pfnCameraSnapToBuffer CameraSnapToBuffer;
        internal static pfnCameraReleaseImageBuffer CameraReleaseImageBuffer;
        internal static pfnCameraPlay CameraPlay;
        internal static pfnCameraPause CameraPause;
        internal static pfnCameraStop CameraStop;
        internal static pfnCameraInitRecord CameraInitRecord;
        internal static pfnCameraStopRecord CameraStopRecord;
        internal static pfnCameraPushFrame CameraPushFrame;
        internal static pfnCameraSaveImage CameraSaveImage;
        internal static pfnCameraGetImageResolution CameraGetImageResolution;
        internal static pfnCameraSetImageResolution CameraSetImageResolution;
        internal static pfnCameraGetMediaType CameraGetMediaType;
        internal static pfnCameraSetMediaType CameraSetMediaType;
        internal static pfnCameraSetAeState CameraSetAeState;
        internal static pfnCameraGetAeState CameraGetAeState;
        internal static pfnCameraSetSharpness CameraSetSharpness;
        internal static pfnCameraGetSharpness CameraGetSharpness;
        internal static pfnCameraSetLutMode CameraSetLutMode;
        internal static pfnCameraGetLutMode CameraGetLutMode;
        internal static pfnCameraSelectLutPreset CameraSelectLutPreset;
        internal static pfnCameraGetLutPresetSel CameraGetLutPresetSel;
        internal static pfnCameraSetCustomLut CameraSetCustomLut;
        internal static pfnCameraGetCustomLut CameraGetCustomLut;
        internal static pfnCameraGetCurrentLut CameraGetCurrentLut;
        internal static pfnCameraSetWbMode CameraSetWbMode;
        internal static pfnCameraGetWbMode CameraGetWbMode;
        internal static pfnCameraSetPresetClrTemp CameraSetPresetClrTemp;
        internal static pfnCameraGetPresetClrTemp CameraGetPresetClrTemp;
		internal static pfnCameraSetUserClrTempGain CameraSetUserClrTempGain;
		internal static pfnCameraGetUserClrTempGain CameraGetUserClrTempGain;
		internal static pfnCameraSetUserClrTempMatrix CameraSetUserClrTempMatrix;
		internal static pfnCameraGetUserClrTempMatrix CameraGetUserClrTempMatrix;
		internal static pfnCameraSetClrTempMode CameraSetClrTempMode;
		internal static pfnCameraGetClrTempMode CameraGetClrTempMode;
        internal static pfnCameraSetOnceWB CameraSetOnceWB;
        internal static pfnCameraSetOnceBB CameraSetOnceBB;
        internal static pfnCameraSetAeTarget CameraSetAeTarget;
        internal static pfnCameraGetAeTarget CameraGetAeTarget;
        internal static pfnCameraSetAeExposureRange CameraSetAeExposureRange;
        internal static pfnCameraGetAeExposureRange CameraGetAeExposureRange;
        internal static pfnCameraSetAeAnalogGainRange CameraSetAeAnalogGainRange;
        internal static pfnCameraGetAeAnalogGainRange CameraGetAeAnalogGainRange;
        internal static pfnCameraSetAeThreshold CameraSetAeThreshold;
        internal static pfnCameraGetAeThreshold CameraGetAeThreshold;
        internal static pfnCameraSetExposureTime CameraSetExposureTime;
        internal static pfnCameraGetExposureLineTime CameraGetExposureLineTime;
        internal static pfnCameraGetExposureTime CameraGetExposureTime;
        internal static pfnCameraGetExposureTimeRange CameraGetExposureTimeRange;
        internal static pfnCameraSetAnalogGain CameraSetAnalogGain;
        internal static pfnCameraGetAnalogGain CameraGetAnalogGain;
        internal static pfnCameraSetGain CameraSetGain;
        internal static pfnCameraGetGain CameraGetGain;
        internal static pfnCameraSetGamma CameraSetGamma;
        internal static pfnCameraGetGamma CameraGetGamma;
        internal static pfnCameraSetContrast CameraSetContrast;
        internal static pfnCameraGetContrast CameraGetContrast;
        internal static pfnCameraSetSaturation CameraSetSaturation;
        internal static pfnCameraGetSaturation CameraGetSaturation;
        internal static pfnCameraSetMonochrome CameraSetMonochrome;
        internal static pfnCameraGetMonochrome CameraGetMonochrome;
        internal static pfnCameraSetInverse CameraSetInverse;
        internal static pfnCameraGetInverse CameraGetInverse;
        internal static pfnCameraSetAntiFlick CameraSetAntiFlick;
        internal static pfnCameraGetAntiFlick CameraGetAntiFlick;
        internal static pfnCameraGetLightFrequency CameraGetLightFrequency;
        internal static pfnCameraSetLightFrequency CameraSetLightFrequency;
        internal static pfnCameraSetFrameSpeed CameraSetFrameSpeed;
        internal static pfnCameraGetFrameSpeed CameraGetFrameSpeed;
        internal static pfnCameraSetParameterMode CameraSetParameterMode;
        internal static pfnCameraGetParameterMode CameraGetParameterMode;
        internal static pfnCameraSetParameterMask CameraSetParameterMask;
        internal static pfnCameraSaveParameter CameraSaveParameter;
        internal static pfnCameraSaveParameterToFile CameraSaveParameterToFile;
        internal static pfnCameraReadParameterFromFile CameraReadParameterFromFile;
        internal static pfnCameraLoadParameter CameraLoadParameter;
        internal static pfnCameraGetCurrentParameterGroup CameraGetCurrentParameterGroup;
        internal static pfnCameraSetTransPackLen CameraSetTransPackLen;
        internal static pfnCameraGetTransPackLen CameraGetTransPackLen;
        internal static pfnCameraIsAeWinVisible CameraIsAeWinVisible;
        internal static pfnCameraSetAeWinVisible CameraSetAeWinVisible;
        internal static pfnCameraGetAeWindow CameraGetAeWindow;
        internal static pfnCameraSetAeWindow CameraSetAeWindow;
        internal static pfnCameraSetMirror CameraSetMirror;
        internal static pfnCameraGetMirror CameraGetMirror;
        internal static pfnCameraSetRotate CameraSetRotate;
        internal static pfnCameraGetRotate CameraGetRotate;
        internal static pfnCameraGetWbWindow CameraGetWbWindow;
        internal static pfnCameraSetWbWindow CameraSetWbWindow;
        internal static pfnCameraIsWbWinVisible CameraIsWbWinVisible;
        internal static pfnCameraSetWbWinVisible CameraSetWbWinVisible;
        internal static pfnCameraImageOverlay CameraImageOverlay;
        internal static pfnCameraSetCrossLine CameraSetCrossLine;
        internal static pfnCameraGetCrossLine CameraGetCrossLine;
        internal static pfnCameraGetCapability _CameraGetCapability;
        internal static pfnCameraWriteSN CameraWriteSN;
        internal static pfnCameraReadSN CameraReadSN;
        internal static pfnCameraSetTriggerDelayTime CameraSetTriggerDelayTime;
        internal static pfnCameraGetTriggerDelayTime CameraGetTriggerDelayTime;
        internal static pfnCameraSetTriggerCount CameraSetTriggerCount;
        internal static pfnCameraGetTriggerCount CameraGetTriggerCount;
        internal static pfnCameraSoftTrigger CameraSoftTrigger;
        internal static pfnCameraSetTriggerMode CameraSetTriggerMode;
        internal static pfnCameraGetTriggerMode CameraGetTriggerMode;
        internal static pfnCameraSetStrobeMode CameraSetStrobeMode;
        internal static pfnCameraGetStrobeMode CameraGetStrobeMode;
        internal static pfnCameraSetStrobeDelayTime CameraSetStrobeDelayTime;
        internal static pfnCameraGetStrobeDelayTime CameraGetStrobeDelayTime;
        internal static pfnCameraSetStrobePulseWidth CameraSetStrobePulseWidth;
        internal static pfnCameraGetStrobePulseWidth CameraGetStrobePulseWidth;
        internal static pfnCameraSetStrobePolarity CameraSetStrobePolarity;
        internal static pfnCameraGetStrobePolarity CameraGetStrobePolarity;
        internal static pfnCameraSetExtTrigSignalType CameraSetExtTrigSignalType;
        internal static pfnCameraGetExtTrigSignalType CameraGetExtTrigSignalType;
        internal static pfnCameraSetExtTrigShutterType CameraSetExtTrigShutterType;
        internal static pfnCameraGetExtTrigShutterType CameraGetExtTrigShutterType;
        internal static pfnCameraSetExtTrigDelayTime CameraSetExtTrigDelayTime;
        internal static pfnCameraGetExtTrigDelayTime CameraGetExtTrigDelayTime;
        internal static pfnCameraSetExtTrigJitterTime CameraSetExtTrigJitterTime;
        internal static pfnCameraGetExtTrigJitterTime CameraGetExtTrigJitterTime;
        internal static pfnCameraGetExtTrigCapability CameraGetExtTrigCapability;
        internal static pfnCameraGetResolutionForSnap CameraGetResolutionForSnap;
        internal static pfnCameraSetResolutionForSnap CameraSetResolutionForSnap;
        internal static pfnCameraCustomizeResolution CameraCustomizeResolution;
        internal static pfnCameraCustomizeReferWin CameraCustomizeReferWin;
        internal static pfnCameraShowSettingPage CameraShowSettingPage;
        internal static pfnCameraCreateSettingPage _CameraCreateSettingPage;
        internal static pfnCameraSetActiveSettingSubPage CameraSetActiveSettingSubPage;
        internal static pfnCameraSpecialControl CameraSpecialControl;
        internal static pfnCameraGetFrameStatistic CameraGetFrameStatistic;
        internal static pfnCameraSetNoiseFilter CameraSetNoiseFilter;
        internal static pfnCameraGetNoiseFilterState CameraGetNoiseFilterState;
        internal static pfnCameraRstTimeStamp CameraRstTimeStamp;
        internal static pfnCameraSaveUserData CameraSaveUserData;
        internal static pfnCameraLoadUserData CameraLoadUserData;
        internal static pfnCameraGetFriendlyName CameraGetFriendlyName;
        internal static pfnCameraSetFriendlyName CameraSetFriendlyName;
        internal static pfnCameraSdkGetVersionString CameraSdkGetVersionString;
        internal static pfnCameraCheckFwUpdate CameraCheckFwUpdate;
        internal static pfnCameraGetFirmwareVision CameraGetFirmwareVision;
        internal static pfnCameraGetEnumInfo CameraGetEnumInfo;
        internal static pfnCameraGetInerfaceVersion CameraGetInerfaceVersion;
        internal static pfnCameraSetIOState CameraSetIOState;
        internal static pfnCameraGetIOState CameraGetIOState;
        internal static pfnCameraSetInPutIOMode CameraSetInPutIOMode;
        internal static pfnCameraSetOutPutIOMode CameraSetOutPutIOMode;
        internal static pfnCameraSetOutPutPWM CameraSetOutPutPWM;
        internal static pfnCameraSetAeAlgorithm CameraSetAeAlgorithm;
        internal static pfnCameraGetAeAlgorithm CameraGetAeAlgorithm;
        internal static pfnCameraSetBayerDecAlgorithm CameraSetBayerDecAlgorithm;
        internal static pfnCameraGetBayerDecAlgorithm CameraGetBayerDecAlgorithm;
        internal static pfnCameraSetIspProcessor CameraSetIspProcessor;
        internal static pfnCameraGetIspProcessor CameraGetIspProcessor;
        internal static pfnCameraSetBlackLevel CameraSetBlackLevel;
        internal static pfnCameraGetBlackLevel CameraGetBlackLevel;
        internal static pfnCameraSetWhiteLevel CameraSetWhiteLevel;
        internal static pfnCameraGetWhiteLevel CameraGetWhiteLevel;
        internal static pfnCameraIsOpened CameraIsOpened;
        internal static pfnCameraEnumerateDeviceEx CameraEnumerateDeviceEx;
        internal static pfnCameraInitEx CameraInitEx;
        internal static pfnCameraInitEx2 CameraInitEx2;
        internal static pfnCameraGetImageBufferEx CameraGetImageBufferEx;
        internal static pfnCameraImageProcessEx CameraImageProcessEx;
        internal static pfnCameraSetIspOutFormat CameraSetIspOutFormat;
        internal static pfnCameraGetIspOutFormat CameraGetIspOutFormat;
        internal static pfnCameraReConnect CameraReConnect;
        internal static pfnCameraConnectTest CameraConnectTest;
        internal static pfnCameraSetLedEnable CameraSetLedEnable;
        internal static pfnCameraGetLedEnable CameraGetLedEnable;
        internal static pfnCameraSetLedOnOff CameraSetLedOnOff;
        internal static pfnCameraGetLedOnOff CameraGetLedOnOff;
        internal static pfnCameraSetLedDuration CameraSetLedDuration;
        internal static pfnCameraGetLedDuration CameraGetLedDuration;
        internal static pfnCameraSetLedBrightness CameraSetLedBrightness;
        internal static pfnCameraGetLedBrightness CameraGetLedBrightness;
        internal static pfnCameraEnableTransferRoi CameraEnableTransferRoi;
        internal static pfnCameraSetTransferRoi CameraSetTransferRoi;
        internal static pfnCameraGetTransferRoi CameraGetTransferRoi;
        internal static pfnCameraAlignMalloc CameraAlignMalloc;
        internal static pfnCameraAlignFree CameraAlignFree;
        internal static pfnCameraSetAutoConnect CameraSetAutoConnect;
        internal static pfnCameraGetReConnectCounts CameraGetReConnectCounts;
        internal static pfnCameraEvaluateImageDefinition CameraEvaluateImageDefinition;
        internal static pfnCameraDrawText CameraDrawText;
        internal static pfnCameraGigeGetIp _CameraGigeGetIp;
        internal static pfnCameraGigeSetIp CameraGigeSetIp;
        internal static pfnCameraGigeGetMac _CameraGigeGetMac;
        internal static pfnCameraEnableFastResponse CameraEnableFastResponse;
        internal static pfnCameraSetCorrectDeadPixel CameraSetCorrectDeadPixel;
        internal static pfnCameraGetCorrectDeadPixel CameraGetCorrectDeadPixel;
        internal static pfnCameraFlatFieldingCorrectSetEnable CameraFlatFieldingCorrectSetEnable;
        internal static pfnCameraFlatFieldingCorrectGetEnable CameraFlatFieldingCorrectGetEnable;
        internal static pfnCameraFlatFieldingCorrectSetParameter CameraFlatFieldingCorrectSetParameter;
        internal static pfnCameraFlatFieldingCorrectSaveParameterToFile CameraFlatFieldingCorrectSaveParameterToFile;
        internal static pfnCameraFlatFieldingCorrectLoadParameterFromFile CameraFlatFieldingCorrectLoadParameterFromFile;
        internal static pfnCameraCommonCall _CameraCommonCall;
        internal static pfnCameraSetDenoise3DParams CameraSetDenoise3DParams;
        internal static pfnCameraGetDenoise3DParams CameraGetDenoise3DParams;
        internal static pfnCameraManualDenoise3D CameraManualDenoise3D;
        internal static pfnCameraCustomizeDeadPixels CameraCustomizeDeadPixels;
        internal static pfnCameraReadDeadPixels CameraReadDeadPixels;
        internal static pfnCameraAddDeadPixels CameraAddDeadPixels;
        internal static pfnCameraRemoveDeadPixels CameraRemoveDeadPixels;
        internal static pfnCameraRemoveAllDeadPixels CameraRemoveAllDeadPixels;
        internal static pfnCameraSaveDeadPixels CameraSaveDeadPixels;
        internal static pfnCameraSaveDeadPixelsToFile CameraSaveDeadPixelsToFile;
        internal static pfnCameraLoadDeadPixelsFromFile CameraLoadDeadPixelsFromFile;
        internal static pfnCameraGetImageBufferPriority CameraGetImageBufferPriority;
        internal static pfnCameraGetImageBufferPriorityEx CameraGetImageBufferPriorityEx;
        internal static pfnCameraGetImageBufferPriorityEx2 CameraGetImageBufferPriorityEx2;
        internal static pfnCameraGetImageBufferPriorityEx3 CameraGetImageBufferPriorityEx3;
        internal static pfnCameraClearBuffer CameraClearBuffer;
        internal static pfnCameraSoftTriggerEx CameraSoftTriggerEx;
        internal static pfnCameraSetHDR CameraSetHDR;
        internal static pfnCameraGetHDR CameraGetHDR;
        internal static pfnCameraGetFrameID CameraGetFrameID;
        internal static pfnCameraGetFrameTimeStamp CameraGetFrameTimeStamp;
        internal static pfnCameraSetHDRGainMode CameraSetHDRGainMode;
        internal static pfnCameraGetHDRGainMode CameraGetHDRGainMode;
        internal static pfnCameraDrawFrameBuffer CameraDrawFrameBuffer;
        internal static pfnCameraFlipFrameBuffer CameraFlipFrameBuffer;
        internal static pfnCameraConvertFrameBufferFormat CameraConvertFrameBufferFormat;
        internal static pfnCameraSetConnectionStatusCallback CameraSetConnectionStatusCallback;
		internal static pfnCameraSetLightingControllerMode CameraSetLightingControllerMode;
		internal static pfnCameraSetLightingControllerState CameraSetLightingControllerState;
        internal static pfnCameraGrabber_CreateFromDevicePage CameraGrabber_CreateFromDevicePage;
        internal static pfnCameraGrabber_CreateByIndex CameraGrabber_CreateByIndex;
        internal static pfnCameraGrabber_CreateByName CameraGrabber_CreateByName;
        internal static pfnCameraGrabber_Create CameraGrabber_Create;
        internal static pfnCameraGrabber_Destroy CameraGrabber_Destroy;
        internal static pfnCameraGrabber_SetHWnd CameraGrabber_SetHWnd;
        internal static pfnCameraGrabber_SetPriority CameraGrabber_SetPriority;
        internal static pfnCameraGrabber_StartLive CameraGrabber_StartLive;
        internal static pfnCameraGrabber_StopLive CameraGrabber_StopLive;
        internal static pfnCameraGrabber_SaveImage CameraGrabber_SaveImage;
        internal static pfnCameraGrabber_SaveImageAsync CameraGrabber_SaveImageAsync;
        internal static pfnCameraGrabber_SaveImageAsyncEx CameraGrabber_SaveImageAsyncEx;
        internal static pfnCameraGrabber_SetSaveImageCompleteCallback CameraGrabber_SetSaveImageCompleteCallback;
        internal static pfnCameraGrabber_SetFrameListener CameraGrabber_SetFrameListener;
        internal static pfnCameraGrabber_SetRawCallback CameraGrabber_SetRawCallback;
        internal static pfnCameraGrabber_SetRGBCallback CameraGrabber_SetRGBCallback;
        internal static pfnCameraGrabber_GetCameraHandle CameraGrabber_GetCameraHandle;
        internal static pfnCameraGrabber_GetStat CameraGrabber_GetStat;
        internal static pfnCameraGrabber_GetCameraDevInfo CameraGrabber_GetCameraDevInfo;
        internal static pfnCameraImage_Create CameraImage_Create;
        internal static pfnCameraImage_CreateEmpty CameraImage_CreateEmpty;
        internal static pfnCameraImage_Destroy CameraImage_Destroy;
        internal static pfnCameraImage_GetData CameraImage_GetData;
        internal static pfnCameraImage_GetUserData CameraImage_GetUserData;
        internal static pfnCameraImage_SetUserData CameraImage_SetUserData;
        internal static pfnCameraImage_IsEmpty CameraImage_IsEmpty;
        internal static pfnCameraImage_Draw CameraImage_Draw;
        internal static pfnCameraImage_DrawFit CameraImage_DrawFit;
        internal static pfnCameraImage_DrawToDC CameraImage_DrawToDC;
        internal static pfnCameraImage_DrawToDCFit CameraImage_DrawToDCFit;
        internal static pfnCameraImage_BitBlt CameraImage_BitBlt;
        internal static pfnCameraImage_BitBltToDC CameraImage_BitBltToDC;
        internal static pfnCameraImage_SaveAsBmp CameraImage_SaveAsBmp;
        internal static pfnCameraImage_SaveAsJpeg CameraImage_SaveAsJpeg;
        internal static pfnCameraImage_SaveAsPng CameraImage_SaveAsPng;
        internal static pfnCameraImage_SaveAsRaw CameraImage_SaveAsRaw;
        internal static pfnCameraImage_IPicture CameraImage_IPicture;
        internal static pfnCameraGetErrorString CameraGetErrorString;
