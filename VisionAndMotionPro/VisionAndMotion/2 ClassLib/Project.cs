    ...
    internal class Project
    {
        ...
        internal static void InportProject()
        {
            try
            {
                ...
                dig_openFileDialog.Title = Configuration.language == Language.English ? "Please select project file" : "请选择项目文件";
                ...
                if (dig_openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    Frm_Main.Instance.OutputMsg(Configuration.language == Language.English ? "Project inported successfully" : "项目导入成功", Color.Green);
                }
            }
            catch (Exception ex)
            {
        ...
        internal static void ExportProject()
        {
            try
            {
                if (Frm_Job.Instance.tbc_jobs.TabCount > 0)
                {
                    ...
                    dig_saveFileDialog.FileName =Configuration .ProgramTitle ;
                    dig_saveFileDialog.Title = Configuration.language == Language.English ? "Please select the project file saving path" : "请选择项目保存路径";
