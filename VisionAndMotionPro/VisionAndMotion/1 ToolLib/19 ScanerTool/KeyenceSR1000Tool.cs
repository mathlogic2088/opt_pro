...
{
    [Serializable]
    internal class KeyenceSR1000Tool : ToolBase
    {
        ...
        internal void Draw_Search_Region()
        {
            try
            {
                ...
                switch (Frm_ShapeMatchTool.Instance.cbx_shapeMatchSearchRegion.Text)
                {
                    ...
                    case "Rectangle1":
                        HTuple row, column, row1, column1;
                        if (searchRegionType == RegionType.Rectangle1 && !Frm_ShapeMatchTool.Instance.ckb_union.Checked)
                            HOperatorSet.DrawRectangle1Mod(GetWindowHandle(jobName), searchRegionPoint[0], searchRegionPoint[1], searchRegionPoint[2], searchRegionPoint[3], out row, out column, out row1, out column1);
                        else
                            HOperatorSet.DrawRectangle1(GetWindowHandle(jobName), out row, out column, out row1, out column1);
                        ...
                        if (Frm_ShapeMatchTool.Instance.ckb_union.Checked && searchRegion != null)
                            HOperatorSet.Union2(searchRegion, rectangle1, out    searchRegion);
                        else
        ...
        internal void Run(bool updateImage, string jobName)
        {
            try
            {
                ...
                if ((int)(rows.TupleLength()) > 0)
                {
                    for (int i = 0; i < (int)rows.TupleLength(); i++)
                    {
                        ...
                        result.Angle = Math.Round((double)(((HTuple)angles[i]).TupleDeg()), 3);
                ...
                if (showFeature)
                {
                    ...
                    for (int i = 0; i < L_matchResult.Count; i++)
                    {
                        ...
                        HOperatorSet.HomMat2dRotate(homMat2D, ((HTuple)L_matchResult[i].Angle).TupleRad(), (HTuple)L_matchResult[i].Row, (HTuple)L_matchResult[i].Col, out homMat2D);
                ...
                for (int i = 0; i < L_matchResult.Count; i++)
                {
                    ...
                    HTuple angle = ((HTuple)(roation)).TupleRad();
                    HOperatorSet.HomMat2dRotate(homMat2D, angle, (HTuple)0, (HTuple)0, out homMat2D);
                    HObject rectangle1AfterTrans;
                    HOperatorSet.HomMat2dTranslate(homMat2D, L_matchResult[i].Row, L_matchResult[i].Col, out homMat2D);
                    HOperatorSet.AffineTransRegion(templateRegion, out rectangle1AfterTrans, homMat2D, "nearest_neighbor");
