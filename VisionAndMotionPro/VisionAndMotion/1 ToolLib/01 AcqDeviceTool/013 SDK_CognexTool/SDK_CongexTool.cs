...
namespace VisionAndMotionPro
{
    ...
    class SDK_CongexTool : ToolBase
    {
        ...
        /// 相机对象
        /// </summary>
        internal Camera_Cognex camera_Cognex = null;
        /// <summary>
        /// 设备描述字符串
        /// </summary>
            ...
            try
            {
                ...
                if (folderBrowseDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    if (L_imageFile.Count > 0)
                    {
                        ...
                        Frm_ReadFromLocalCognex.Instance.lbl_imageName.Text = currentImageName;
                    }
                    Frm_ReadFromLocalCognex.Instance.lbl_imageNum.Text = "共" + L_imageFile.Count + "张";
        ...
        internal void Real_Time_Acquistion()
        {
            try
            {
                if (camera_Cognex == null)
                {
                    Frm_SDK_CognexTool.Instance.tsb_realTimeDisplay.CheckState = CheckState.Unchecked;
                ...
                {
                    realTimeMode = true;
                    Frm_SDK_CognexTool.Instance.tsb_realTimeDisplay.CheckState = CheckState.Checked;
                    Frm_AcqFromDeviceCognex.Instance.btn_RealTime.Text = "停止实时";
                    Frm_AcqFromDeviceCognex.Instance.btn_runSDKCognexTool.Enabled = false;
                    Frm_AcqFromDeviceCognex.Instance.btn_saveImage.Enabled = false;
                    Frm_AcqFromDeviceCognex.Instance.cbx_deviceList.Enabled = false;
                    Frm_AcqFromDeviceCognex.Instance.btn_registImage.Enabled = false;
                    Frm_AcqFromDeviceCognex.Instance.tbx_exposure.Enabled = false;
                    Frm_AcqFromDeviceCognex.Instance.btn_runSDKCognexTool.BackColor = Color.LightGray;
                    Frm_AcqFromDeviceCognex.Instance.btn_saveImage.BackColor = Color.LightGray;
                    Frm_AcqFromDeviceCognex.Instance.btn_registImage.BackColor = Color.LightGray;
                ...
                else
                {
                    realTimeMode = false;
                    Frm_SDK_CognexTool.Instance.tsb_realTimeDisplay.CheckState = CheckState.Unchecked;
                    Frm_AcqFromDeviceCognex.Instance.btn_RealTime.Text = "实时采集";
                    Frm_AcqFromDeviceCognex.Instance.btn_runSDKCognexTool.Enabled = true;
                    Frm_AcqFromDeviceCognex.Instance.btn_saveImage.Enabled = true;
                    Frm_AcqFromDeviceCognex.Instance.btn_registImage.Enabled = true;
                    Frm_AcqFromDeviceCognex.Instance.cbx_deviceList.Enabled = true;
                    Frm_AcqFromDeviceCognex.Instance.btn_runSDKCognexTool.BackColor = Color.White;
                    Frm_AcqFromDeviceCognex.Instance.btn_saveImage.BackColor = Color.White;
                    Frm_AcqFromDeviceCognex.Instance.btn_registImage.BackColor = Color.White;
                    Frm_AcqFromDeviceCognex.Instance.tbx_exposure.Enabled = true;
                }
            }
            catch (Exception ex)
            {
        ...
        internal void Device_Changed(string deviceInfoStr1)
        {
            try
            {
                if (Frm_AcqFromDeviceCognex.Instance.cbx_deviceList.Text != string.Empty)
                {
                    //////for (int i = 0; i < Camera_Cognex.L_devices.Count; i++)
                    //////{
                    //////    if (Camera_Cognex.L_devices[i].DeviceDescriptionStr  == deviceInfoStr1)
                    //////    {
                    //////        Frm_AcqFromDeviceCognex.Instance.tkb_exposure.Maximum = Camera_Cognex.L_devices[i].MaxExposure;
                    //////        Frm_AcqFromDeviceCognex.Instance.tkb_exposure.Minimum = Camera_Cognex.L_devices[i].MinExposure;   
                    //////        double exposure = camera_Cognex.ExposureTime;
                    //////        Frm_AcqFromDeviceCognex.Instance.tbx_exposure.Text = exposure.ToString();
                    //////        //////Frm_AcqFromDeviceCognex.Instance.tkb_exposure.Value = exposure;
                    //////        Frm_AcqFromDeviceCognex.Instance.lbl_exposureRange.Text = "曝光范围：" + Camera.L_device[i].MinExposure + " ~ " + Camera.L_device[i].MaxExposure;
                    //////        this.camera_Cognex = Camera_Cognex.L_devices[i].camera;
                    //////        this.deviceInfoStr = deviceInfoStr1;
                    //////    }
                    //////    else
                    //////    {
                    //////        this.camera_Cognex = null;
                    //////    }
                    //////}
                    Frm_Main.Save();
                }
        ...
        internal void Set_Exposure(string jobName)
        {
            try
            {
                ...
                if (camera_Cognex != null)
                {
                    Int32 exposure = Convert.ToInt32(Frm_AcqFromDeviceCognex.Instance.tbx_exposure.Text.Trim());
                    for (int i = 0; i < Camera.L_device.Count; i++)
                    {
                        if (Camera.L_device[i].DeviceDescriptionStr == Frm_AcqFromDeviceCognex.Instance.cbx_deviceList.Text.Trim())
                        {
                    ...
                    if (Frm_AcqFromDeviceCognex.Instance.tkb_exposure.Value != exposure)
                        Frm_AcqFromDeviceCognex.Instance.tkb_exposure.Value = exposure;
                    camera_Cognex.ExposureTime = exposure;
                    Thread.Sleep(100);
                    if (!Frm_Main.ignore)
        ...
        public  override  void Run(string jobName, bool updateImage, bool b)
        {
            ...
            if (imageSourceMode == ImageSourceMode.FormDevice)      //从设备采集图像
            {
                ...
                try
                {
                    HOperatorSet.GrabImage(out outputImage, new HTuple(camera_Cognex));
                }
                catch
                {
                    camera_Cognex = Camera_Cognex.Get_Camera(deviceInfoStr);
                    HOperatorSet.GrabImage(out outputImage, new HTuple(camera_Cognex));
                }
                if (updateImage)
