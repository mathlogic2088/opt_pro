...
namespace VisionAndMotionPro
{
    ...
    class SDK_BaslerTool : ToolBase
    {
        ...
        /// 相机对象
        /// </summary>
        internal Camera_Basler camera_Basler = null;
        /// <summary>
        /// 设备描述字符串
        /// </summary>
            ...
            try
            {
                ...
                if (folderBrowseDialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    if (L_imageFile.Count > 0)
                    {
                        ...
                        Frm_ReadFromLocalBasler.Instance.lbl_imageName.Text = currentImageName;
                    }
                    Frm_ReadFromLocalBasler.Instance.lbl_imageNum.Text = "共" + L_imageFile.Count + "张";
                    Frm_Main.Save();
                }
            }
            catch (Exception ex)
        ...
        internal void Real_Time()
        {
            try
            {
                while (realTimeMode)
                {
                    outputImage = camera_Basler.GrabOneImage();
        ...
        internal void Real_Time_Acquistion()
        {
            try
            {
                ...
                {
                    Frm_SDK_BaslerTool.Instance.tsb_realTimeDisplay.CheckState = CheckState.Unchecked;
                ...
                {
                    realTimeMode = true;
                    Frm_SDK_BaslerTool.Instance.tsb_realTimeDisplay.CheckState = CheckState.Checked;
                    Frm_AcqFromDeviceBasler.Instance.btn_RealTime.Text = "停止实时";
                    Frm_AcqFromDeviceBasler.Instance.btn_runSDKBaslerTool.Enabled = false;
                    Frm_AcqFromDeviceBasler.Instance.btn_saveImage.Enabled = false;
                    Frm_AcqFromDeviceBasler.Instance.cbx_deviceList.Enabled = false;
                    Frm_AcqFromDeviceBasler.Instance.btn_registImage.Enabled = false;
                    Frm_AcqFromDeviceBasler.Instance.tbx_exposure.Enabled = false;
                    Frm_AcqFromDeviceBasler.Instance.btn_runSDKBaslerTool.BackColor = Color.LightGray;
                    Frm_AcqFromDeviceBasler.Instance.btn_saveImage.BackColor = Color.LightGray;
                    Frm_AcqFromDeviceBasler.Instance.btn_registImage.BackColor = Color.LightGray;
                ...
                else
                {
                    realTimeMode = false;
                    Frm_SDK_BaslerTool.Instance.tsb_realTimeDisplay.CheckState = CheckState.Unchecked;
                    Frm_AcqFromDeviceBasler.Instance.btn_RealTime.Text = "实时采集";
                    Frm_AcqFromDeviceBasler.Instance.btn_runSDKBaslerTool.Enabled = true;
                    Frm_AcqFromDeviceBasler.Instance.btn_saveImage.Enabled = true;
                    Frm_AcqFromDeviceBasler.Instance.btn_registImage.Enabled = true;
                    Frm_AcqFromDeviceBasler.Instance.cbx_deviceList.Enabled = true;
                    Frm_AcqFromDeviceBasler.Instance.btn_runSDKBaslerTool.BackColor = Color.White;
                    Frm_AcqFromDeviceBasler.Instance.btn_saveImage.BackColor = Color.White;
                    Frm_AcqFromDeviceBasler.Instance.btn_registImage.BackColor = Color.White;
                    Frm_AcqFromDeviceBasler.Instance.tbx_exposure.Enabled = true;
                }
            }
            catch (Exception ex)
            {
        ...
        internal void Device_Changed(string deviceInfoStr)
        {
            try
            {
                if (Frm_AcqFromDeviceBasler.Instance.cbx_deviceList.Text != string .Empty )
                {
                    for (int i = 0; i < Camera_Basler.L_devices.Count; i++)
                    {
                        if (Camera_Basler.L_devices[i].DeviceInfoStr == deviceInfoStr)
                        {
                            camera_Basler = Camera_Basler.L_devices[i].Camera_basler;
                            Frm_AcqFromDeviceBasler.Instance.tkb_exposure.Maximum = Camera_Basler.L_devices[i].MaxExposure;
                            Frm_AcqFromDeviceBasler.Instance.tkb_exposure.Minimum = Camera_Basler.L_devices[i].MinExposure;
                            Int32 exposure = camera_Basler.ExposureTime;
                            Frm_AcqFromDeviceBasler.Instance.tbx_exposure.Text = exposure.ToString();
                            Frm_AcqFromDeviceBasler.Instance.tkb_exposure.Value = (int)exposure;
                            Frm_AcqFromDeviceBasler.Instance.lbl_exposureRange.Text = "曝光范围：" + Camera_Basler.L_devices[i].MinExposure + " ~ " + Camera_Basler.L_devices[i].MaxExposure;
        ...
        internal void Set_Exposure(string jobName)
        {
            try
            {
                ...
                if (camera_Basler != null)
                {
                    Int32 exposure = Convert.ToInt32(Frm_AcqFromDeviceBasler.Instance.tbx_exposure.Text.Trim());
                    for (int i = 0; i < Camera_Basler.L_devices.Count; i++)
                    {
                        if (Camera_Basler.L_devices[i].DeviceInfoStr == Frm_AcqFromDeviceBasler.Instance.cbx_deviceList.Text.Trim())
                        {
                            if (Camera_Basler.L_devices[i].MinExposure <= exposure && exposure <= Camera_Basler.L_devices[i].MaxExposure)
                                Camera_Basler.L_devices[i].Exposure = exposure;
                            else
                            {
                    ...
                    if (Frm_AcqFromDeviceBasler.Instance.tkb_exposure.Value != exposure)
                        Frm_AcqFromDeviceBasler.Instance.tkb_exposure.Value = exposure;
        ...
        public  override  void Run( string jobName,bool updateImage,bool b)
        {
            ...
            if (imageSourceMode == ImageSourceMode.FormDevice)      //从设备采集图像
            {
                ...
                {
                    camera_Basler = Camera_Basler.Get_Camera(deviceInfoStr);
                    outputImage = camera_Basler.GrabOneImage();
                }
                if (updateImage)
            ...
            else
            {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        {
                            Frm_Main.Instance.OutputMsg(Configuration.language == Language.English ? "The path is invalid(ErrorCode:0106)" : "路径不合法（错误代码：0106）", Color.Red);
                            return;
                        }
                        for (int i = 0; i < files.Length; i++)
                        {
